...
window.exportPDF = async function () {
  const stock =
    ...
    (window.AppState ? window.AppState.stockData : null);
  if (!stock) {
    if (window.showToast)
      showToast("Load stock data before exporting.", "error");
    return;
  }
  ...
  if (typeof window.html2pdf !== "function") {
    if (window.showToast)
      showToast(
        "PDF export library did not load. Check your internet connection and retry.",
        "error",
      );
    return;
  }
  ...
  headerDiv.style.cssText =
    "border-bottom: 2px solid #e2e8f0; padding-bottom: 16px; margin-bottom: 24px; display: block;";
  ...
  try {
    ...
    if (window.showToast)
      showToast("Could not create the PDF. Please retry.", "error");
  } finally {
