...
function openSettingsModal() {
  document.getElementById('inputGroqKey').value = localStorage.getItem('GROQ_API_KEY') || '';
  document.getElementById('inputGeminiKey').value = localStorage.getItem('GEMINI_API_KEY') || '';
  document.getElementById('inputOpenRouterKey').value = localStorage.getItem('OPENROUTER_API_KEY') || '';
...
function saveSettings() {
  localStorage.setItem('GROQ_API_KEY', document.getElementById('inputGroqKey').value.trim());
  localStorage.setItem('GEMINI_API_KEY', document.getElementById('inputGeminiKey').value.trim());
  localStorage.setItem('OPENROUTER_API_KEY', document.getElementById('inputOpenRouterKey').value.trim());
  closeSettingsModal();
...
window.fetch = async (...args) => {
  ...
  // Create a new Headers object or plain object to safely modify
...
async function fetchMarketIndices() {
  try {
    ...
    const container = document.getElementById("marketIndicesBar");
      ...
      container.innerHTML = data
        .map((idx) => {
          ...
          const svg = isPos 
            ...
            : `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 17 13.5 8.5 8.5 13.5 2 7"></polyline><polyline points="16 17 22 17 22 11"></polyline></svg>`;
          return `
          <div class="market-ticker-pill ${cls}" style="cursor: pointer;" onclick="els.searchInput.value='${idx.name === 'S&P 500' ? '^GSPC' : idx.name === 'NASDAQ' ? '^IXIC' : idx.name === 'DOW JONES' ? '^DJI' : '^VIX'}'; els.searchForm.dispatchEvent(new Event('submit'));">
            <span class="idx-name">${idx.name}</span>
            <span class="idx-price">${fmt(idx.price, "price")}</span>
            <span class="idx-change" style="display: flex; align-items: center; gap: 4px;">
              ${svg}
              ${fmt(Math.abs(idx.change) / 100, "percent")}
            </span>
          </div>
        `;
        })
    ...
    const container = document.getElementById("marketIndicesBar");
    if (container) {
      container.innerHTML = ["S&P 500", "NASDAQ", "DOW JONES", "VIX (Vol)"].map(name => `
        <div class="market-ticker-pill" style="cursor: pointer;" onclick="els.searchInput.value='${name === 'S&P 500' ? '^GSPC' : name === 'NASDAQ' ? '^IXIC' : name === 'DOW JONES' ? '^DJI' : '^VIX'}'; els.searchForm.dispatchEvent(new Event('submit'));">
          <span class="idx-name">${name}</span>
          <span class="idx-price">---</span>
          <span class="idx-change" style="display: flex; align-items: center; gap: 4px; color: var(--text-muted);">
            0.00%
          </span>
        </div>
      `).join("");
...
function injectSkeletons() {
  ...
  if (hero)
    hero.innerHTML = `<div class="skeleton" style="height: 120px; width: 100%;"></div>`;
}



// ── App Init Loading Helper ───────────────────────
function showLoading(on) {
  ...
  if (!overlay) {
    ...
    overlay.style.cssText =
      "position:fixed;inset:0;z-index:9000;background:rgba(6,10,18,0.75);display:none;align-items:center;justify-content:center;backdrop-filter:blur(4px);";
  ...
  try {
    ...
    if (messagesEl)
      messagesEl.innerHTML =
        '<div style="color: var(--text-muted); font-style: italic;">Ask a question to begin...</div>';
...
function transformPillarsViewModel(data) {
  ...
  const defPeriod =
    data.data_years_available > 0 && data.data_years_available < 5
      ? `${data.data_years_available}-Year`
      : "5-Year";
  const defPeriodShort =
    data.data_years_available > 0 && data.data_years_available < 5
      ? `${data.data_years_available}-Yr`
      : "5-Yr";
  const getPeriod = (key) => data[`${key}_meta`]?.period || defPeriodShort;

  const rawPillars = [
    {
      name: `${defPeriod} Median P/E`,
      val: data.pillar_pe_5yr,
      fmt: "multiple",
      thresh: "< 22.5×",
      pass: isValid(data.pillar_pe_5yr) && data.pillar_pe_5yr < 22.5,
    },
    {
      name: `${defPeriod} Avg ROIC`,
      val: data.pillar_roic_5yr,
      fmt: "percent",
      thresh: "> 9%",
      pass: isValid(data.pillar_roic_5yr) && data.pillar_roic_5yr > 0.09,
    },
    {
      name: `Shares Trend (${getPeriod("shares_cagr_5yr")} CAGR)`,
      val: data.shares_cagr_5yr,
      fmt: "percent",
      thresh: "Declining",
      pass:
        typeof sharesTrend === "boolean"
          ? sharesTrend
          : isValid(data.shares_cagr_5yr) && data.shares_cagr_5yr < 0,
    },
    {
      name: `FCF Growth (${getPeriod("fcf_cagr_5yr")} CAGR)`,
      val: data.fcf_cagr_5yr,
      fmt: "percent",
      thresh: "Positive",
      pass: isValid(data.fcf_cagr_5yr) && data.fcf_cagr_5yr > 0,
    },
    {
      name: `Net Income Growth (${getPeriod("netincome_cagr_5yr")} CAGR)`,
      val: data.netincome_cagr_5yr,
      fmt: "percent",
      thresh: "Positive",
      pass: isValid(data.netincome_cagr_5yr) && data.netincome_cagr_5yr > 0,
    },
    {
      name: `Revenue Growth (${getPeriod("revenue_cagr_5yr")} CAGR)`,
      val: data.revenue_cagr_5yr,
      fmt: "percent",
      thresh: "Positive",
      pass: isValid(data.revenue_cagr_5yr) && data.revenue_cagr_5yr > 0,
    },
    {
      name: `LT Liabilities / ${defPeriodShort} FCF`,
      val: data.ltl_5yr_fcf_ratio,
      fmt: "decimal",
      thresh: "< 5.0×",
      pass: isValid(data.ltl_5yr_fcf_ratio) && data.ltl_5yr_fcf_ratio < 5.0,
    },
    {
      name: `${defPeriod} Median P/FCF`,
      val: data.pillar_pfcf_5yr,
      fmt: "multiple",
      thresh: "< 22.5×",
      pass: isValid(data.pillar_pfcf_5yr) && data.pillar_pfcf_5yr < 22.5,
    },
  ];
...
function renderHero(data) {
  ...
  if (watchIcon) {
    const isWatched =
      AppState.watchlist && AppState.watchlist.includes(vm.ticker);
...
function renderAnalystTargets(data) {
  ...
  document.getElementById("target-low-val").textContent =
    "$" + targetLow.toFixed(2);
  document.getElementById("target-avg-val").textContent =
    "$" +
    (targetMean
      ? targetMean.toFixed(2)
      : ((targetLow + targetHigh) / 2).toFixed(2));
  document.getElementById("target-high-val").textContent =
...
function renderSummaryMetrics(data) {
  ...
  const m = [
    ...
    [
      `${defPeriodShort} Avg Profit Margin`,
      data.avg_netincome_margin_5yr,
      "percent",
      true,
    ],
    ...
    [
      `${getPeriod("revenue_cagr_5yr")} Revenue Growth`,
    ...
    [
      `${defPeriodShort} Avg FCF Margin`,
      data.avg_fcf_margin_5yr,
      "percent",
      true,
    ],
    ["Shares Outstanding", data.shares_outstanding, "shares", false],
    [
      `Shares CAGR (${getPeriod("shares_cagr_5yr")})`,
      data.shares_cagr_5yr,
      "percent",
      true,
    ],
  ...
  document.getElementById("metricsContainer").innerHTML = m
    .map(([label, val, type, color]) => {
...
function renderHistoricalTable(data) {
  ...
  const annual = (data.annual || []).filter(
    (a) => a.revenue != null || a.netIncome != null,
  );
  if (!annual.length) return;
  const hasFiveYears = annual.length === 5;
  document.getElementById("finDataYearsLabel").textContent =
    `${annual.length} years of annual data`;
  ...
  document.getElementById("finTableBody").innerHTML = rows
    .map(
      (r) =>
        `<tr><td>${r.label}</td>${annual
          .map((a) => {
            const val = a[r.key];
            const cls = r.color
              ? colorCls(val)
              : typeof val === "number" && val < 0 && r.fmt === "currency"
                ? "text-danger"
                : "";
...
function renderSharesChart(data) {
  if (AppState.sharesChart) {
    AppState.sharesChart.destroy();
    AppState.sharesChart = null;
  }

  const annualShares = (data.annual_shares || [])
    .slice()
  ...
  const bgColor = isBuyback
    ? "rgba(34, 197, 94, 0.2)"
    : "rgba(239, 68, 68, 0.2)";

  // Sync Legend Status
  const subtitleEl = document
    .querySelector("#sharesChart")
    .closest(".card")
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          bodyColor: "#e2e8f0",
          callbacks: {
            label: (ctx) => ` ${ctx.dataset.label}: ${fmt(ctx.raw, "shares")}`,
          },
        },
      },
      scales: {
  ...
  if (quarterlyShares.length) {
    document.getElementById("sharesTableHead").innerHTML =
      "<th>Period</th>" +
      quarterlyShares.map((q) => `<th>${q.quarter}</th>`).join("");
    document.getElementById("sharesTableBody").innerHTML =
      "<tr><td>Diluted Shares</td>" +
      quarterlyShares
        .map((q) => `<td class="mono">${fmt(q.shares, "shares")}</td>`)
        .join("") +
      "</tr>";
  }
}
...
function populateValuationDefaults(data) {
  document.getElementById("valDataYears").textContent =
    `${data.data_years_available || 0} Yrs Data`;
  ...
  const bRows = [
    ...
    [
      ...
      data.revenue_cagr_5yr,
      data.revenue_cagr_10yr,
    ],
    [
      "Net Income Margin",
      data.netincome_margin_1yr,
      data.avg_netincome_margin_5yr,
      data.avg_netincome_margin_10yr,
    ],
    [
      "FCF Margin",
      data.fcf_margin_1yr,
      data.avg_fcf_margin_5yr,
      data.avg_fcf_margin_10yr,
    ],
    [
      "Gross Margin",
      data.gross_margin_1yr,
      data.gross_margin_5yr,
      data.gross_margin_10yr,
    ],
  ];
  document.getElementById("benchmarksBody").innerHTML = bRows
    .map(
      ([lbl, v1, v5, v10]) =>
        `<tr><td>${lbl}</td>
...
function resetValuationDefaults() {
  ...
  v.low = {
    revGrowth: pct(def.low_revenue_growth),
    niMargin: pct(def.low_ni_margin),
    fcfMargin: pct(def.low_fcf_margin),
    pe: val(def.low_pe),
    pfcf: val(def.low_pfcf),
    sharesGrowth: pct(def.low_shares_growth),
    discountRate: pct(def.low_discount_rate),
  };
  v.mid = {
    revGrowth: pct(def.mid_revenue_growth),
    niMargin: pct(def.mid_ni_margin),
    fcfMargin: pct(def.mid_fcf_margin),
    pe: val(def.mid_pe),
    pfcf: val(def.mid_pfcf),
    sharesGrowth: pct(def.mid_shares_growth),
    discountRate: pct(def.mid_discount_rate),
  };
  v.high = {
    revGrowth: pct(def.high_revenue_growth),
    niMargin: pct(def.high_ni_margin),
    fcfMargin: pct(def.high_fcf_margin),
    pe: val(def.high_pe),
    pfcf: val(def.high_pfcf),
    sharesGrowth: pct(def.high_shares_growth),
    discountRate: pct(def.high_discount_rate),
  };
...
function renderAssumptionsTable() {
  ...
  document.getElementById("assumptionsBody").innerHTML = rows
    .map(
      (r) => `
...
function calculateValuation() {
  ...
  ["low", "mid", "high"].forEach((s) => {
    ...
    const valid =
      [rev_g, ni_m, fcf_m, pe, pfcf, sh_g, dr].every(Number.isFinite) &&
      rev_g > -1 &&
      rev_g <= 1 &&
      Math.abs(ni_m) <= 1 &&
      Math.abs(fcf_m) <= 1 &&
...
function renderValuationResults(res, price) {
  ...
  const mos = (fv) =>
    isValid(fv) && fv > 0 ? ((fv - price) / fv) * 100 : null;
  const valTxt = (d) =>
...
function filterNews(category) {
  ...
  if (
    AppState.newsStatus === "unavailable" &&
    !(AppState.newsData || []).length
  ) {
...
function downloadCSV() {
  ...
  if (!table) {
    if (window.showToast)
      showToast("No financial table found to export.", "error");
    return;
  }
  ...
  const ticker =
    window.AppState && AppState.currentTicker
      ? AppState.currentTicker
      : "Stock";
...
async function renderWatchlistDashboard() {
  ...
  const summaryStrip = document.getElementById("watchlistSummary");
  const emptyState = document.getElementById("watchlistEmptyState");
  const statusMsg = document.getElementById("watchlistStatusMessage");
  const tableBody = document.getElementById("watchlistTableBody");

  if (!tableContainer) return; // Not on the watchlist page or layout not updated yet
  ...
  if (!AppState.watchlist || AppState.watchlist.length === 0) {
    if (localWatchlist.length > 0) {
      AppState.watchlist = localWatchlist;
    } else {
      hideAll();
  ...
  hideAll();
  statusMsg.innerHTML = "Fetching live quotes and metrics...";
  ...
  try {
    ...
    // Sort quotes by change percent descending to find top gainer
    ...
    document.getElementById("wlTotalTickers").textContent = quotes.length;
    if (topGainer && topGainer.change > 0) {
      document.getElementById("wlTopGainer").innerHTML = `<span style="color: var(--success); font-weight: 600;">${topGainer.ticker} (+${topGainer.changePercent.toFixed(2)}%)</span>`;
    } else if (topGainer) {
      document.getElementById("wlTopGainer").textContent = `${topGainer.ticker} (${topGainer.changePercent.toFixed(2)}%)`;
    } else {
      document.getElementById("wlTopGainer").textContent = "--";
    }
    ...
    tableBody.innerHTML = quotes
      .map((q) => {
        ...
        const mcap = q.market_cap ? fmt(q.market_cap, "currency") : "--";
        const pe = q.pe_ratio ? fmt(q.pe_ratio, "decimal") + "x" : "--";
        
        const formatDayChange = (changeVal, changePct) => {
          const isPositive = changeVal > 0;
          const sign = isPositive ? '+' : '';
          const formattedVal = `${sign}${Math.abs(changeVal).toFixed(2)}`;
          const formattedPct = `${sign}${Math.abs(changePct).toFixed(2)}%`;
          let result = `${formattedVal} (${formattedPct})`;
          if (!isPositive && changeVal !== 0) {
            result = `-${result}`;
          }
        ...
        return `
        <tr style="cursor: pointer; transition: background 0.2s;" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background='transparent'" onclick="els.searchInput.value='${q.ticker}'; els.searchForm.dispatchEvent(new Event('submit'));">
          <td style="font-weight: 700; font-family: var(--font-mono); color: var(--text-main);">${q.ticker}</td>
          <td style="color: var(--text-muted); max-width: 200px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${q.company_name || q.ticker}</td>
          <td style="text-align: right; font-weight: 600; font-family: var(--font-mono);">${fmt(q.price, "price")}</td>
          <td style="text-align: right; color: ${color}; font-family: var(--font-mono); font-weight: 500;">
            ${formatDayChange(q.change, q.changePercent)}
          </td>
          <td style="text-align: right; font-family: var(--font-mono); color: var(--text-muted);">${mcap}</td>
          <td style="text-align: right; font-family: var(--font-mono); color: var(--text-muted);">${pe}</td>
          <td style="text-align: center;">
            <button onclick="event.stopPropagation(); removeWatchlistItem('${q.ticker}');" class="btn btn-sm" style="padding: 4px 8px; background: transparent; border: 1px solid var(--border); color: var(--text-muted);" title="Remove">🗑️</button>
          </td>
        </tr>
      `;
      })
    ...
    hideAll();
    statusMsg.innerHTML = `<div style="color: var(--danger); margin-bottom: 16px;">Failed to fetch live quotes.</div>
      <button class="btn btn-outline" onclick="renderWatchlistDashboard()">Retry</button>`;
    statusMsg.style.display = "block";
  }
}
...
async function removeWatchlistItem(ticker) {
  try {
    ...
    if (res.ok) {
      AppState.watchlist = await res.json();
      if (AppState.currentTicker === ticker) renderHero(AppState.stockData);
...
async function fetchChartData(range, interval) {
  ...
  try {
    const res = await fetch(
      `/api/chart/${encodeURIComponent(AppState.currentTicker)}?range=${range}&interval=${interval}`,
    );
    ...
    if (subtitle)
      subtitle.innerHTML = `<span style="color:var(--danger)">Failed to load chart data</span>`;
  }
}
...
if (searchInput) {
  searchInput.addEventListener("input", (e) => {
    ...
    searchTimeout = setTimeout(async () => {
      try {
        searchAutocomplete.innerHTML = '<div style="padding:10px; text-align:center; color:var(--text-muted);">Searching...</div>';
        ...
        searchAutocomplete.innerHTML = results
          .map(
            (r) => {
              const typeBadge = r.type ? `<span style="font-size: 0.7rem; background: var(--surface); padding: 2px 6px; border-radius: 4px; border: 1px solid var(--border-bright); color: var(--text-muted); margin-left: auto;">${r.type}</span>` : "";
              return `
          <div class="autocomplete-item" style="padding:10px 12px; cursor:pointer; border-bottom:1px solid var(--border-color); display: flex; align-items: center; gap: 8px;" onclick="selectAutocomplete('${r.symbol}')" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background='transparent'">
            <strong style="color: var(--text-main); min-width: 60px;">${r.symbol}</strong> 
            <span class="text-muted" style="font-size:0.9em; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${r.name}</span>
            ${typeBadge}
          </div>
        `})
          .join("");
...
// ── AI SWOT Analysis & Company News ───────────────────────
window.fetchSWOTAnalysis = async function(data) {
  const quadrants = ['strengths', 'weaknesses', 'opportunities', 'threats'];
  const resetSkeletons = () => {
    quadrants.forEach(q => {
      const el = document.getElementById(`swot-${q}`);
      if (el) el.innerHTML = `<div class="skeleton-text" style="width: 90%;"></div><div class="skeleton-text" style="width: 75%;"></div>`;
    });
  };
  ...
  try {
    const payload = {
      ...
      roic: data.roic_ttm,
      revenue_growth: data.rev_cagr_5yr,
      debt_to_equity: data.debt_equity,
      recent_headlines: data.news ? data.news.slice(0, 3).map(n => n.title).join(" | ") : ""
    };
    ...
    if (!res.ok) throw new Error("Failed to fetch SWOT");
    const swot = await res.json();
    
    quadrants.forEach(q => {
      const el = document.getElementById(`swot-${q}`);
      if (el && swot[q]) {
        el.innerHTML = swot[q].map(item => `<li>${item}</li>`).join("");
      }
    });
  } catch (err) {
    ...
    quadrants.forEach(q => {
      const el = document.getElementById(`swot-${q}`);
      if (el) el.innerHTML = `<li>Error loading ${q}</li>`;
    });
  }
};

window.renderStockNews = function(data) {
  const container = document.getElementById("companyNewsContainer");
  if (!container) return;
  
  if (!data.news || data.news.length === 0) {
    container.innerHTML = `<div style="color: var(--text-muted); font-size: 0.9rem;">No recent news available.</div>`;
    return;
  }
  
  const newsHtml = data.news.slice(0, 5).map(item => {
    const date = new Date(item.providerPublishTime * 1000).toLocaleDateString();
    return `
      <a href="${item.link}" target="_blank" style="text-decoration: none; color: inherit; display: block; padding: 12px; background: var(--bg2); border: 1px solid var(--border); border-radius: 8px; transition: transform 0.2s;">
        <div style="font-weight: 600; font-size: 0.95rem; margin-bottom: 4px;">${item.title}</div>
        <div style="font-size: 0.8rem; color: var(--text-muted);">${item.publisher} • ${date}</div>
      </a>
    `;
  }).join("");
...
window.fetchMarketMovers = async function(category = 'gainers') {
  const container = document.getElementById("marketMoversContent");
  if (!container) return;
  
  document.querySelectorAll("[id^='btnMovers']").forEach(btn => btn.classList.remove("active"));
  const btn = document.getElementById(`btnMovers${category.charAt(0).toUpperCase() + category.slice(1)}`);
  if (btn) btn.classList.add("active");
  
  container.innerHTML = `<div class="skeleton-text" style="width: 100%; height: 24px;"></div><div class="skeleton-text" style="width: 100%; height: 24px;"></div><div class="skeleton-text" style="width: 100%; height: 24px;"></div>`;
  
  try {
    const res = await fetchWithRetry("/api/market_movers");
    if (!res.ok) throw new Error();
    const data = await res.json();
    ...
    container.innerHTML = items.map(item => {
      ...
      const cls = isPos ? "pos" : "neg";
      const sign = isPos ? "+" : "";
      const valStr = category === 'volume' 
        ? `<span style="color: var(--text-main); font-weight: 500;">${fmt(item.volume, "shares")}</span>`
        : `<span class="${cls}" style="font-weight: 500;">${sign}${item.change.toFixed(2)}%</span>`;
        
      return `
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px 0; border-bottom: 1px solid rgba(255,255,255,0.05); cursor: pointer;" onclick="els.searchInput.value='${item.symbol}'; els.searchForm.dispatchEvent(new Event('submit'));" onmouseover="this.style.background='rgba(255,255,255,0.02)'" onmouseout="this.style.background='transparent'">
          <div style="display: flex; align-items: center; gap: 12px;">
            <strong style="color: var(--text-main); width: 50px;">${item.symbol}</strong>
            <span style="color: var(--text-muted); font-size: 0.85rem; max-width: 120px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${item.name}</span>
          </div>
          <div>
            ${valStr}
          </div>
    ...
    const fallbackMovers = [
      { symbol: "AAPL", name: "Apple Inc.", change: 1.25, volume: 55000000 },
      { symbol: "MSFT", name: "Microsoft Corp.", change: 0.85, volume: 22000000 },
      { symbol: "NVDA", name: "NVIDIA Corp.", change: 2.10, volume: 45000000 }
    ];
    container.innerHTML = fallbackMovers.map(item => {
      const isPos = item.change >= 0;
      const cls = isPos ? "pos" : "neg";
      const sign = isPos ? "+" : "";
      const valStr = category === 'volume' 
...
window.renderHomeWatchlistPreview = async function() {
  ...
  if (!AppState.watchlist || AppState.watchlist.length === 0) {
    if (localWatchlist.length > 0) {
      ...
      container.innerHTML = `
        <div style="text-align: center; color: var(--text-muted); padding: 16px 0; font-size: 0.95rem;">
          No saved tickers. 
          <div style="margin-top: 12px; display: flex; justify-content: center; gap: 12px;">
            <button class="btn btn-sm btn-outline" onclick="window.addSampleToWatchlist('NVDA', 'NVIDIA Corp')">+ NVDA</button>
            <button class="btn btn-sm btn-outline" onclick="window.addSampleToWatchlist('AAPL', 'Apple Inc')">+ AAPL</button>
          </div>
        </div>`;
      return;
    }
  ...
  container.innerHTML = `<div style="color: var(--text-muted); font-size: 0.9rem; padding: 8px 0;">Fetching live quotes...</div>`;
  try {
    const res = await window.fetchWithRetry("/api/watchlist/quotes");
    if (!res.ok) throw new Error();
    const quotes = await res.json();
    ...
    const getCompanyName = (ticker) => {
      const match = AppState.watchlist.find(w => w.symbol === ticker);
      return match ? match.name : "";
    };
    
    container.innerHTML = quotes.slice(0, 4).map(q => {
      const isUp = q.change >= 0;
      const color = isUp ? "var(--positive)" : "var(--negative)";
      const bg = isUp ? "rgba(16, 185, 129, 0.1)" : "rgba(239, 68, 68, 0.1)";
      const sign = isUp ? "+" : "";
      const companyName = getCompanyName(q.ticker);
      
      return `
        <div style="display: flex; align-items: center; padding: 10px 12px; border-bottom: 1px solid rgba(255,255,255,0.05); cursor: pointer; transition: background 0.2s;" 
             onclick="els.searchInput.value='${q.ticker}'; els.searchForm.dispatchEvent(new Event('submit'));" 
          ...
          <div style="flex: 1; display: flex; align-items: center; gap: 12px; overflow: hidden;">
            <strong style="color: var(--text-main); font-size: 0.95rem; min-width: 50px;">${q.ticker}</strong>
            <span style="color: var(--text-muted); font-size: 0.85rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px;">${companyName}</span>
          </div>
    ...
    container.innerHTML = `<div style="color: var(--danger); font-size: 0.9rem;">Failed to fetch watchlist quotes.</div>`;
  }
};

window.addSampleToWatchlist = function(symbol, name) {
  let current = JSON.parse(localStorage.getItem("watchlist") || "[]");
  if (!current.find(w => w.symbol === symbol)) {
    current.push({symbol, name});
    localStorage.setItem("watchlist", JSON.stringify(current));
    AppState.watchlist = current;
    window.renderHomeWatchlistPreview();
  }
};
