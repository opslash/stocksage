...
def fetch_macro_news() -> dict:
    ...
    def get_sentiment(title):
        t = title.lower()
        if any(
            w in t for w in ["rate increase", "hike", "tighten", "inflation concern"]
        ):
            return "hawkish"
        ...
        for e in feed.entries:
            if e.link not in seen_urls:
                ...
                if "cpi" in t or "inflation" in t:
                    mtype = "inflation"
                elif "unemployment" in t or "payroll" in t:
                    mtype = "employment"
                elif "ppi" in t:
                    mtype = "producer_prices"
                news_items.append(
                    {
                        "title": e.title,
                        "url": e.link,
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            for article in resp.json().get("articles", []):
                ...
                if (
                    any(v in src.lower() for v in valid_sources)
            ...
            news_items.sort(key=lambda x: x["published"], reverse=True)
            return {
                "status": "partial" if news_items else "unavailable",
