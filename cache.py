...
def save_cache(key: str, data, ttl_seconds: int = None):
    if redis_client:
        try:
            redis_client.set(key, json.dumps({"timestamp": time.time(), "data": data}), ex=ttl_seconds)
            return
        except Exception as e:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile('w', delete=False, dir=dir_name, suffix='.tmp', encoding='utf-8') as tf:
            json.dump({"timestamp": time.time(), "data": data, "ttl_seconds": ttl_seconds}, tf)
            tmp_path = tf.name
...
def load_cache(key: str, max_age_minutes: int = 75):
    ...
    if key != "news" and not key.startswith("peers_") and not key.startswith("fin_") and isinstance(data, dict):
