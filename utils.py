...
def get_first_valid_val(
    df: pd.DataFrame, row_names: List[str], col: str
) -> Optional[float]:
...
def best_cagr(
    data_list: List[Dict[str, Any]], key: str, max_n: int
...
def normalize_shares_history(
    history_list: List[Dict[str, Any]], key: str = "shares"
) -> List[Dict[str, Any]]:
    """
            ...
            if ratio and (ratio >= split_high or ratio <= split_low):
                try:
                    split_factor = (
                        round(ratio)
                        if ratio > 1
                        else (1 / round(1 / ratio) if round(1 / ratio) != 0 else ratio)
                    )
